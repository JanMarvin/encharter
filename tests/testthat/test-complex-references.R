...
test_that("Chart: Multi-level labels and Bubbles", {
  ...
  chart <- Chart$new()
  expect_error(chart$add_series(header = "A1", data = "C2:C5"), "Series data must be a sheet reference")
  chart$add_series(header = "A1", data = "dat!C2:C5", cat = "dat!A2:B5")
  ...
  bb$add_series(header = "H", data = "dat!B2:B5", cat = "dat!A2:A5", z_data = "dat!C2:C5")
