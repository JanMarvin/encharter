...
seatbelts_final <- seatbelts_clean |>
  mutate(
    ...
    , fatality_rate_12m_weighted = slide_dbl(
      .x = seatbelts_clean,
      .f = ~ {
        # Weighted by distance: sum(weights * ratio) / sum(weights)
...
e$add_series(data = wd, name = `Before law change`, label = date, type = "barplot", secondary = TRUE, line_color = wb_color(hex = scales::alpha("lightgray", alpha = 0.2), format = "RGBA"), gap_width = 0, overlap = 100)
e$add_series(data = wd, name = `Weighted 12m Fatality Rate`, label = date, secondary = TRUE, type = "line", line_color = wb_color("orange"), line_width = 2)
e$add_series(data = wd, name = `12m Fatality Rate`, label = date, secondary = TRUE, type = "line", line_color = wb_color("darkgreen"), line_width = 2)
