...
[![CRAN status](https://www.r-pkg.org/badges/version/encharter)](https://cran.r-project.org/package=encharter)
...
- Series cell references use standard spreadsheet notation: `"Sheet1!$A$2:$A$10"`. The sheet name is required.
- For extended chart types (waterfall, treemap, etc.), OOXML uses a different XML schema internally (`chartEx`). `encharter` handles this transparently — `ec("waterfall")` returns a `ChartEx` object rather than a `Chart` object, but the interface is the same.
